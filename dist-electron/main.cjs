        ...
        exec(cmd, (error, stdout, stderr) => {
          if (error) {
            ...
            if (cmd.startsWith("setupc") && !cmd.includes("\\") && !cmd.includes("/")) {
              const localSetupc = path.join(electron.app.getPath("userData"), "drivers", "com0com", "setupc.exe");
...
let tcpService = null;
electron.ipcMain.handle("tcp:start", async (_event, port) => {
  if (!tcpService) return { success: false, error: "Service not initialized" };
  return tcpService.startServer(port);
});
electron.ipcMain.handle("tcp:stop", async (_event, port) => {
  if (!tcpService) return false;
  return tcpService.stopServer(port);
});
electron.ipcMain.handle("tcp:write", async (_event, { port, data }) => {
