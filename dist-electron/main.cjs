...
const stateFile = path$m.join(require$$1$3.app.getPath("userData"), "window-state.json");
const saveState = () => {
...
function createWindow() {
  ...
  require$$1$3.ipcMain.handle("serial:list-ports", async () => {
    return serialService == null ? void 0 : serialService.listPorts();
  });
  require$$1$3.ipcMain.handle("serial:open", async (_event, { connectionId, options }) => {
    return serialService == null ? void 0 : serialService.open(connectionId, options);
  });
  require$$1$3.ipcMain.handle("serial:close", async (_event, { connectionId }) => {
    return serialService == null ? void 0 : serialService.close(connectionId);
  });
  require$$1$3.ipcMain.handle("serial:write", async (_event, { connectionId, data }) => {
    return serialService == null ? void 0 : serialService.write(connectionId, data);
  });
  const mqtt = require("mqtt");
  const mqttClients = /* @__PURE__ */ new Map();
  require$$1$3.ipcMain.handle("mqtt:connect", async (_event, { connectionId, config }) => {
    return new Promise((resolve) => {
      ...
      const options = {
        ...
        wsOptions: {
          ...
          headers: {
            "User-Agent": `SerialTool/${require$$1$3.app.getVersion()}`
          }
        }
      };
      ...
      const handleInitialSuccess = () => {
        if (!initialConnectHandled) {
          ...
          if (config.topics && Array.isArray(config.topics)) {
            config.topics.forEach((t2) => client.subscribe(t2));
          }
        }
      };
  ...
  require$$1$3.ipcMain.handle("mqtt:disconnect", async (_event, { connectionId }) => {
  ...
  require$$1$3.ipcMain.handle("mqtt:publish", async (_event, { connectionId, topic, payload, options }) => {
    const client = mqttClients.get(connectionId);
    if (client) {
      return new Promise((resolve) => {
  ...
  require$$1$3.ipcMain.handle("mqtt:subscribe", async (_event, { connectionId, topic }) => {
    const client = mqttClients.get(connectionId);
    if (client) {
  ...
  require$$1$3.ipcMain.handle("mqtt:unsubscribe", async (_event, { connectionId, topic }) => {
    const client = mqttClients.get(connectionId);
    if (client) {
  ...
  require$$1$3.ipcMain.handle("com0com:exec", async (_event, command) => {
    return new Promise((resolve) => {
      if (!command.startsWith("setupc")) {
        return resolve({ success: false, error: "Unauthorized command" });
      }
      const execCommand = (cmd) => {
        exec(cmd, (error2, stdout, stderr) => {
          if (error2) {
            console.log(`[com0com] Command failed: ${cmd}`, error2.message);
            if (cmd.startsWith("setupc") && !cmd.includes("\\") && !cmd.includes("/")) {
              const localSetupc = path$m.join(require$$1$3.app.getPath("userData"), "drivers", "com0com", "setupc.exe");
              console.log(`[com0com] Trying local path: ${localSetupc}`);
              const { spawn } = require("node:child_process");
              const dir = path$m.dirname(localSetupc);
              ...
              const child = spawn(localSetupc, args, {
                cwd: dir,
                shell: true
                // Needed? setupc is .exe. usually ok without if full path. but for UAC maybe?
                // If shell: true, verify if we need to quote args. spawn handles array args well usually.
              });
              let out2 = "";
              let errOut = "";
              child.stdout.on("data", (d) => out2 += d.toString());
              ...
              child.on("close", (code) => {
                if (code === 0) {
                  ...
                  resolve({ success: true, stdout: out2 });
                } else {
              ...
              resolve({ success: false, error: error2.message, stderr });
            }
          } else {
  ...
  require$$1$3.ipcMain.handle("com0com:install", async () => {
    const isDev = !!VITE_DEV_SERVER_URL;
    let installerPath = "";
    if (isDev) {
      installerPath = path$m.join(__dirname, "../resources/drivers/com0com_setup.exe");
    } else {
      installerPath = path$m.join(process.resourcesPath, "resources/drivers/com0com_setup.exe");
    }
    const targetDir = path$m.join(require$$1$3.app.getPath("userData"), "drivers", "com0com");
    try {
      const stats = await fs2.stat(installerPath);
      if (!stats.isFile()) {
    ...
    return new Promise((resolve) => {
      ...
      const child = spawn(installerPath, ["/S", `/D=${targetDir}`], {
        windowsHide: false,
        shell: true,
        cwd: path$m.dirname(installerPath)
      });
      child.on("error", (err) => {
        resolve({ success: false, error: err.message });
      });
  ...
  const updater = new AppUpdater2(win);
  updater.init();
}
class AppUpdater2 {
  constructor(win2) {
  ...
  init() {
    ...
    require$$1$3.ipcMain.handle("update:check", () => {
      return main$1.autoUpdater.checkForUpdates();
    });
    require$$1$3.ipcMain.handle("update:download", () => {
      return main$1.autoUpdater.downloadUpdate();
    });
    require$$1$3.ipcMain.handle("update:install", () => {
...
class TcpService {
  constructor(webContents) {
    ...
    __publicField(this, "webContents");
...
let tcpService = null;
require$$1$3.ipcMain.handle("tcp:start", async (_event, port) => {
  if (!tcpService) return { success: false, error: "Service not initialized" };
  return tcpService.startServer(port);
});
require$$1$3.ipcMain.handle("tcp:stop", async (_event, port) => {
  if (!tcpService) return false;
  return tcpService.stopServer(port);
});
require$$1$3.ipcMain.handle("tcp:write", async (_event, { port, data }) => {
